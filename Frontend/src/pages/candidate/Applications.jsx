    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          ...
          <Link
            to="/jobs"
            className="inline-flex items-center gap-2 px-4 py-2.5 bg-emerald-500 hover:bg-emerald-400 text-slate-950 font-bold rounded-lg text-sm transition"
          >
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-2xl overflow-hidden backdrop-blur-xl shadow-xl">
          <div className="divide-y divide-slate-800/80">
            {applications.map((app) => (
              <div key={app.id} className="p-5 sm:p-6 hover:bg-slate-800/40 transition flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="space-y-1.5">
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <span className="font-bold text-base text-white">{app.jobTitle}</span>
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold border ${app.pillColor}`}>
                      {app.statusPill}
                    </span>
                  </div>
                  <div className="text-xs text-slate-400 flex items-center gap-4">
                    <span>Company: <strong className="text-slate-300">{app.company}</strong></span>
                ...
                <div className="flex items-center gap-3 shrink-0">
                  <Link
                    to={app.actionUrl}
                    className="px-4 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-emerald-400 text-xs font-semibold transition inline-flex items-center gap-1.5 border border-slate-700/60"
                  >
        ...
        {/* Info Box */}
        <div className="mt-8 p-4 bg-emerald-950/20 border border-emerald-500/20 rounded-xl flex items-start gap-3 text-xs text-slate-300">
