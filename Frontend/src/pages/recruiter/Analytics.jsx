  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 backdrop-blur-xl">
          <div className="text-xs text-slate-400 uppercase font-semibold">Average JD Neutrality</div>
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 backdrop-blur-xl">
          <div className="text-xs text-slate-400 uppercase font-semibold">Average Time-to-Hire</div>
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 backdrop-blur-xl">
          <div className="text-xs text-slate-400 uppercase font-semibold">Completed Assessments</div>
          <div className="text-2xl font-bold text-white mt-1">{data.assessmentsCompleted}</div>
        ...
        <div className="bg-slate-900/70 border border-slate-800 rounded-xl p-5 backdrop-blur-xl">
          <div className="text-xs text-slate-400 uppercase font-semibold">Demographic Parity Rate</div>
          <div className="text-2xl font-bold text-white mt-1">99.4%</div>
          <div className="text-[11px] text-slate-400 mt-2">
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6 backdrop-blur-xl space-y-4">
        <h2 className="text-base font-bold text-white flex items-center gap-2">
          <Users className="w-4 h-4 text-emerald-400" /> End-to-End Blind Evaluation Funnel
        </h2>
      ...
      <div className="bg-slate-900/70 border border-slate-800 rounded-2xl p-6 backdrop-blur-xl">
        <h2 className="text-base font-bold text-white mb-4 flex items-center gap-2">
          <ShieldCheck className="w-4 h-4 text-emerald-400" /> Bias Reduction by Department
        </h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {data.biasReductionByDept.map((d, i) => (
            <div key={i} className="p-4 bg-slate-950/60 rounded-xl border border-slate-800/80 space-y-2 text-xs">
              <div className="flex justify-between font-bold text-white text-sm">
                <span>{d.dept}</span>
                <span className="text-emerald-400">{d.postBias}/100 Neutral</span>
              </div>
              <div className="flex items-center gap-3 text-slate-400">
                <span>Raw Draft: {d.preBias}%</span>
                <span className="text-emerald-400 font-semibold">➔ Post-Scan: {d.postBias}%</span>
              </div>
              <div className="w-full bg-slate-900 rounded-full h-1.5 overflow-hidden">
                <div className="bg-emerald-400 h-1.5 rounded-full" style={{ width: `${d.postBias}%` }} />
              </div>
            </div>
          ))}
        </div>
